...
public class Viewer extends GUIState{
	...
	public void setupPortrayals(){
	    ...
	    for(Class c : objects){
	    	try {
				knownWorld.setPortrayalForClass(c, (Portrayal) c.getMethod("getPortrayal", null).invoke(null,null));//new RectanglePortrayal2D(Color.orange,1));
						//c.getMethod("getPortrayal", null).invoke(null,null));
			} catch (Exception e){
				e.printStackTrace();
			}
	    }
